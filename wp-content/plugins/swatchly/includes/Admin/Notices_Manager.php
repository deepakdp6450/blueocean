...
class Swatchly_Notice_Manager{

    // Remote URL
    const REST_ROUTE_URL = 'https://feed.hasthemes.com/notices';
    // const REST_ROUTE_URL = SWATCHLY_URL;

    // Transient Key
    const TRANSIENT_KEYS = [
        'notice'  => 'swatchly_notice_info',
    ];

    // API Endpoint
    const API_ENDPOINT = [
        'notice'      => 'swatchly.json',
    ];
    ...
    public static function get_api_endpoint(){
        if( is_plugin_active('swatchly-pro/swatchly-pro.php') && function_exists('swatchly_pro_notice_endpoint') ){
            return swatchly_pro_notice_endpoint();
        }
    ...
    public static function delete_transient_cache_data(){
        if ( get_option( 'swatchly_delete_data_fetch_cache', FALSE ) ) {
