...
class HTSlider_Notice_Manager{

    // Remote URL
    const REST_ROUTE_URL = 'https://feed.hasthemes.com/notices';
     //const REST_ROUTE_URL = HTSLIDER_PL_URL;

    // Transient Key
    const TRANSIENT_KEYS = [
        'notice'  => 'htslider_notice_info',
    ];

    // API Endpoint
    const API_ENDPOINT = [
        'notice'      => 'htslider-notice.json',
    ];
    ...
    public static function get_api_endpoint(){
        if( is_plugin_active('htslider-pro/init.php') && function_exists('htslider_pro_notice_endpoint') ){
            return htslider_pro_notice_endpoint();
        }
    ...
    public static function delete_transient_cache_data(){
        if ( get_option( 'htslider_delete_data_fetch_cache', FALSE ) ) {
            foreach( self::TRANSIENT_KEYS as $transient_key ){
                delete_transient( $transient_key );
            }
            delete_option('htslider_delete_data_fetch_cache');
        }
    }
