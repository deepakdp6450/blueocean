...
class HTSlider_Template_Library{
    ...
    public function scripts( $hook ) {
        ...
        wp_localize_script(
            ...
            [
                ...
                'nonce'            => wp_create_nonce('ht_slider_template_lib_nonce'),
                'user'             => [
    ...
    function templates_ajax_request(){
        ...
        check_ajax_referer('ht_slider_template_lib_nonce', 'nonce');

        if ( ! current_user_can( 'manage_options') ) {
                echo wp_json_encode(
                    array(
                        'message' => esc_html__( 'You are not permitted to import the template.', 'ht-slider' )
                    )
                );
        }else{
