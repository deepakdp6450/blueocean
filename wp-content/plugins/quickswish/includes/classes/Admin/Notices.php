    ...
    */
    ...
    public function admin_notice_missing_woocommerce_plugin(){
        ...
        if( $this->is_plugins_active( $woocommerce ) ) {
            ...
            $message = sprintf( __( '%1$sQuickSwish%2$s requires %1$s"WooCommerce"%2$s plugin to be active. Please activate WooCommerce to continue.', 'quickswish' ), '<strong>', '</strong>');
            $button_text = __( 'Activate WooCommerce', 'quickswish' );
        } else {
            if( ! current_user_can( 'activate_plugins' ) ) {
                return;
            }
            ...
            $message = sprintf( __( '%1$sQuickSwish%2$s requires %1$s"WooCommerce"%2$s plugin to be installed and activated. Please install WooCommerce to continue.', 'quickswish' ), '<strong>', '</strong>' );
            $button_text = __( 'Install WooCommerce', 'quickswish' );
        }
