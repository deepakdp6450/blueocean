...
class PayuAccountAddressSync extends PayuPaymentGatewayAPI
{
    ...
    public function schedule_account_address_push($user_id, $address_type)
    {
        ...
        $payu_address_data = $wpdb->get_row(
            $wpdb->prepare(
                // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name uses trusted WordPress table prefix.
                "SELECT payu_address_id, payu_user_id
                FROM {$wp_payu_address_table}
                WHERE user_id = %d
                AND address_type = %s",
                $user_id,
                $address_type
        ...
        if ($payu_address_data) {
            // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified by WooCommerce before this hook is fired.
    ...
    public function custom_woocommerce_shipping_fields($fields)
    {
        ...
        $fields['shipping_phone'] = array(
            'label'     => __('Phone Number', 'payu-india'),
            'required'  => true,
            'class'     => array('form-row-wide'),
            'clear'     => true,
        );
    ...
    {
        global $wpdb;
        $wp_payu_address_table = $wpdb->prefix . 'payu_address_sync';
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $address_sync_data = $wpdb->get_results(
            $wpdb->prepare(
                // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name uses trusted WordPress table prefix.
                "SELECT address_type
                FROM {$wp_payu_address_table}
    ...
    public function payu_address_sync_brefore_payment()
    {
        ...
        $sync_data =  $this->check_payu_address_sync($user_id);
