...
class Manage_Compare {
    ...
    public function pop_up_html(){
        echo '<div class="htcompare-popup"><div class="htcompare-popup-content-area"><button type="button" class="htcompare-popup-close" aria-label="' . esc_attr__( 'Close compare popup', 'ever-compare' ) . '">&nbsp;</button><div class="htcompare-table" data-ajax-load="true"></div></div></div>';
    }
        ...
        $ids = $this->get_compared_products();
         ...
         if ( ! empty( $_GET['evcompare'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended
            ...
            if( !empty( $query_perametter_ids ) ){
                $ids = array_filter( array_map( 'absint', explode( ',', $query_perametter_ids ) ) );
            }
        }
    ...
    public function get_compared_products_data() {
        ...
        // For shareable link
        $shareable_link = ever_compare_get_option('enable_shareable_link','ever_compare_table_settings_tabs','off');
        if ( ( $shareable_link === 'on' ) && ! empty( $_GET['evcompare'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $query_perametter_ids = sanitize_text_field( wp_unslash($_GET['evcompare']) ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended
            if( !empty( $query_perametter_ids ) ){
                $ids = array_filter( array_map( 'absint', explode( ',', $query_perametter_ids ) ) );
            }
            ...
            case 'primary':
                ?>
                    <div class="htcompare-primary-content-area">
                        ...
                        <a href="#" class="htcompare-remove" data-product_title="<?php echo esc_attr( $product['title'] ); ?>" data-product_id="<?php echo esc_attr( $product['id'] ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Remove %s from compare', 'ever-compare' ), $product['title'] ) ); ?>">&nbsp;</a>
    ...
    public function availability_html( $product ) {
        ...
        if( empty( $availability['availability'] ) ) {
            $availability['availability'] = __( 'In stock', 'ever-compare' );
        }
    ...
    public function get_compared_products() {
        ...
        if(!empty( $_COOKIE[ $cookie_name ] )) {
            $decoded = json_decode( sanitize_text_field(wp_unslash( $_COOKIE[ $cookie_name ] )), true );
            if ( is_array( $decoded ) ) {
