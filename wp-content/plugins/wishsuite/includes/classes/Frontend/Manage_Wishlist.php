...
class Manage_Wishlist {
    ...
    public function add_product( $id ){
        ...
        if( $user_id ){
            ...
            $temp_user_id = !empty($_COOKIE[$temp_id_key]) ? sanitize_text_field($_COOKIE[$temp_id_key]) : false;
            if(!$temp_user_id) {
                $temp_user_id = time();
            }
    ...
    public function display_field( $field_id, $product ) {
        ...
        switch ( $type ) {
            case 'remove':
                ?>
                    <a href="#" class="wishsuite-remove" data-product-title="<?php echo esc_attr($product[ 'title' ]); ?>" data-product_id="<?php echo esc_attr( $product['id'] ); ?>">&nbsp;</a>
                <?php
                break;
