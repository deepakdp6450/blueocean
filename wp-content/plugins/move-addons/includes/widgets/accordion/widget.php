...
class Accordion_Element extends Base {
        ...
        $this->start_controls_section(
            ...
            $this->add_control(
                ...
                [
                    ...
                    'type'      => Controls_Manager::COLOR,
                    'selectors' => [
                        '{{WRAPPER}} .htmove-accordion .htmove-accordion-card .htmove-accordion-content,{{WRAPPER}} .htmove-accordion .htmove-accordion-card .htmove-accordion-content p' => 'color: {{VALUE}};',
                    ],
                    'separator' => 'before',
                ]
            );
    ...
    protected function render( $instance = [] ) {
        ...
        $this->add_render_attribute( 'area_attr', 'class', 'htmove-accordion htmove-icon-pos-'.esc_attr($settings['icon_position']) );
        $this->add_render_attribute( 'area_attr', 'id', 'htmove-accordion-'.esc_attr($id) );
                ...
                <?php
                    if( is_array( $accordion_list ) ){
                        foreach ( $accordion_list as  $accordion ){
                            ...
                            $title = ( !empty( $accordion['title'] ) ? '<span class="htmove-accordion-head-text">'.esc_html($accordion['title']).'</span>' : '' );
